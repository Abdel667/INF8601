...
__kernel void sinoscope_image_kernel(
    ...
    __constant sinoscope_int_values_t* sinoscope_int_values,
    __constant sinoscope_float_values_t* sinoscope_float_values)
{
    int i = get_global_id(1); // Row index
    int j = get_global_id(0); // Column index

    if (i < sinoscope_int_values->height && j < sinoscope_int_values->width) {
        ...
        pixel_t pixel;
        color_value(&pixel, value, sinoscope_int_values->interval, sinoscope_int_values->interval_inverse);

        int index = (i * sinoscope_int_values->width + j) * 3;
